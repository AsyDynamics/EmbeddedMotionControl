...
Feature::AveragedLaserData Feature::Preprocess(Feature::AveragedLaserData &laser) { //check input! check function type!
    ...
    for (int i=0;i<SCAN_POINTS;i++){
        if(std::abs(laser.angle[i])<=PI/2 && std::isnan(laser.angle[i])==false)//check if nan happens  && isnan(laser.angle[i]==false)
        {
            ...
            ProcessedData.vx.push_back(cos(laser.angle[i])*laser.range[i]+laser.x); // absolute xy coord
            ProcessedData.vy.push_back(sin(laser.angle[i])*laser.range[i]+laser.y);
        }
    }
    ProcessedData.x = laser.x;
    ProcessedData.y = laser.y;
    std::cout<<"Odom theta is: "<<laser.theta<<std::endl;
...
// ******************************* Merge ************************************
Feature::AveragedLaserData Feature::Merge(Feature::AveragedLaserData &flaser, Feature::AveragedLaserData &blaser){
...
// ******************************************** Merge Map of different scan ****************************************
bool Feature::MergeMap(std::vector<std::vector<double> > &Map, std::vector<std::vector<double> > &cornermatrix){
    for (double i=0;i<cornermatrix.size();i++){
        Map.push_back(cornermatrix[i]);
    }
    std::cout<<"The total secion number of world is: "<<Map.size()<<std::endl;
}

// ******************************************** save map as csv ****************************************************
bool saveMap(const char *file, const std::vector<std::vector<double> > &matrix, const Feature::AveragedLaserData &laser) {

    std::ofstream filestream(file);

    if (filestream.is_open()) {
        for (double i=0;i<matrix.size();i++){
            for (double j=0;j<matrix[i].size();j++){
                double temp = matrix[i][j];
                filestream << temp <<","<<laser.vx[temp]<<","<<laser.vy[temp]<<","; // csv, 1c:index, 2c: x coord, 3c: y coord
            }
            filestream<< "\n";
        }
        filestream.close();
