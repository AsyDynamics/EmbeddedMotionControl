...
public:
    ...
    AveragedLaserData Preprocess(AveragedLaserData &laser, bias Bias);
    AveragedLaserData MergeScan(AveragedLaserData &flaser, AveragedLaserData &blaser, bias bias);
    ...
    std::vector<std::vector<double> > FeatureDetection2(AveragedLaserData &laser, section_index &section_info);
    ...
    bool MergeMap(std::vector<std::vector<CornerInfo> > &cornerStruct, std::vector<std::vector<CornerInfo> > &Map);
    std::vector<CornerInfo> findDoor(std::string mode, std::vector<std::vector<CornerInfo> > &cornerStruct);
    ...
    std::vector<std::vector<CornerInfo> > index2Struct(std::string mode,std::vector<std::vector<double> > cornermatrix, AveragedLaserData &laser, double y1, double y2);
};
